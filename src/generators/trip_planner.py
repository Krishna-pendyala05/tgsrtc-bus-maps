...
======================
Generates an interactive trip planning tool with route finding.

Features:
- Location-based search (not directional stops)
- Direct routes and single-transfer options
...
class TripPlannerGenerator(BaseGenerator):
    """Generator for the trip planner HTML."""
    
    output_filename = "trip_planner.html"
    
    def generate(self) -> str:
        """Generate the trip planner HTML."""
        
        self._log_progress("Building routing indices...")
        indices = self._build_indices()
        
        self._log_progress(f"Generating HTML with {len(indices['locationCoords'])} locations...")
        return self._generate_html(indices)
    
    def _extract_base_name(self, stop_name: str) -> str:
        """
    ...
    def _build_indices(self) -> dict:
        ...
        self._log_progress("  Building stop location index...")
        ...
        for _, row in stops.iterrows():
            ...
            stop_base_names[sid] = self._extract_base_name(full_name)
            try:
        ...
        self._log_progress(f"    {len(stop_names):,} stops, {len(location_stops):,} locations")
        ...
        # 5. Route-Stops sequences (using representative trips)
        self._log_progress("  Building route-stops sequences...")
        ...
        # 8. Travel times
        self._log_progress("  Calculating travel times...")
        ...
        for rid, times in route_stop_times.items():
            ...
            for i in range(len(times) - 1):
                ...
                if t1 is not None and t2 is not None:
                    ...
                    if diff < 0:
                        diff = 3
                    durations.append(max(1, min(diff, 30)))
                else:
                    durations.append(3)
    ...
    def _generate_html(self, indices: dict) -> str:
        ...
        return self._get_html_template(data_json, locations_json)
    
    def _get_html_template(self, data_json: str, locations_json: str) -> str:
