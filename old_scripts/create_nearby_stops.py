"""
Enhanced version - Bus Stops Near Me with Routes for each stop
Processes GTFS data to map stops to routes
"""
import pandas as pd
import json
...
# Create route_id -> route_long_name mapping
...
print(f"Mapped {len(stop_routes)} stops to routes")

# Sample check
sample_stop = list(stop_routes.keys())[0]
print(f"Sample: Stop {sample_stop} has routes: {list(stop_routes[sample_stop])[:5]}...")
...
print(f"\nCreating HTML with {len(stops_data)} stops...")
...
<body>
    ...
    <script>
        // All bus stops data with routes
        const STOPS = ''' + json.dumps(stops_data) + ''';
...
# Save the file
with open('nearby_stops.html', 'w', encoding='utf-8') as f:
    f.write(html_content)

print(f"Created nearby_stops.html")
print(f"File size: {len(html_content) / 1024 / 1024:.2f} MB")

# Stats
routes_per_stop = [len(s['routes']) for s in stops_data]
print(f"\nStats:")
print(f"  Average routes per stop: {sum(routes_per_stop)/len(routes_per_stop):.1f}")
print(f"  Max routes at a stop: {max(routes_per_stop)}")
print(f"  Stops with routes: {sum(1 for r in routes_per_stop if r > 0)}")
